...
dat <- read.delim("data_scen14.txt")
...
library(plotly) 
library(umap) 
umap = umap(dat[,1:5], n_components = 2, random_state = 15, n_neigbours=15) 
layout <- umap[["layout"]] 
...
fviz_cluster (pam_result_s, 
              palette = c("#00AFBB", "#FC4E07", "#2E9FDF","#9E0142"), # color palette
...
ri_tk_s<-rand.index(dat$X1, t_K$cluster)
...
# kmeans       0.31878988 0.6243017 0.1100104 0.0021034324
# tclust      -0.06842004 0.7390798 0.5808915 0.0032318705
#    PAM       0.31489009 0.6218744 0.1098236 0.0009937894
# mclust      -0.04082400 0.8881547 0.7504227 0.0013475818
#tkmeans       0.27584667 0.6291783 0.1890299 0.0046122447
library(xtable)
...
#        Method Silhouette_Score        RI       ARI       DI
# t-SNE + kmeans        0.6167216 0.6526206 0.1802076 0.000125
# t-SNE + tclust        0.4659818 0.6650619 0.3226543 0.000320
#    t-SNE + PAM        0.6168854 0.6525664 0.1796899 0.000206
# t-SNE + mclust        0.6167216 0.6424280 0.1641111 0.000083
#t-SNE + tkmeans        0.5901465 0.6580087 0.2533029 0.000171
...
ri_umap_tk_s<-randIndex(dat$X1, u_t_K$cluster)
...
Metrics_table_int_scaled <- data.frame(
  ...
  ARI=c(umap_kmeans_ari,umap_tclust_ari,umap_pam_ari,umap_mc_ari, utk_ari),
...
#        Method Silhouette_Score        RI       ARI DI
# UMAP + kmeans        0.5995448 0.2072699 0.2072699  0
# UMAP + tclust        0.3047822 0.3473295 0.4509474  0
#    UMAP + PAM        0.5775239 0.1905278 0.1905278  0
# UMAP + mclust        0.7085716 0.2979852 0.2979852  0
#UMAP + tkmeans        0.5901465 0.2109838 0.2518557 0.00068
...
#MDS+tclust
dis_ss = dist(mds_result)^2
clus_s <- tclust(mds_result, k = 4, alpha = 0.1, restr.fact = 50)
